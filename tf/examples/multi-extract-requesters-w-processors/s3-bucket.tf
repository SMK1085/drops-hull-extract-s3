...
resource "aws_iam_policy" "lambda_hull_extract_processor" {
  ...
  description = "Grants a Hull Extract Processor full access to the given S3 bucket"
  policy = <<POLICY
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_role_policy_attachment" "lambda_hull_extractor_s3_access2" {
  role       = module.lambda_hull_extract_processor2.role_name   # Reference to the role_name output from the main module
  policy_arn = aws_iam_policy.lambda_hull_extract_processor.arn
}